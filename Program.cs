using System.Collections;   //пространство имен для работы с объектом Stack
...
for (int i = 0; i < str?.Length; i++)
{
    ...
    {
        Console.WriteLine("Введенное выражение содержит некорректные символы");
        return;
    }
}
...
if (count != 0)
{
    Console.WriteLine("Введенное выражение не содержит одинаковое кол-во открывающихся и закрывающихся скобок");
    return;
}
...
for (int i = 0; i < str?.Length; i++)
{
        ...
        str[i] == '/')
    {
        ...
        else
        {
            ...
            //или открывающаяся скобка или ничего нет, то заносим в стек
            if (temp.Count == 0 ||
            ...
            {
                //знаки операций из стека, пока приоритет их больше или равен приоритету данного знака,
                //извлекаются из стека и заносятся в результирующую строку
...
if (!flag)
{
    ...
    Console.WriteLine($"Вычисленное выражение: {temp.Pop()}");
}